...
def india_short_yields():
    """India short-tenor yields from FRED. FRED does not carry India 1Y/2Y G-Sec
    yields, so we use the India 3-month T-bill rate (IR3TTS01INM156N) as the
    short-end anchor and interpolate the 2Y from the spread to 10Y.
    Returns {'1Y': float, '2Y': float} or {}."""
    if not FRED_API_KEY:
        return {}
    out = {}
    # India 3-month T-bill rate (closest govt security short-tenor on FRED)
    for sid in ("IR3TTS01INM156N", "IRSTCI01INM156N"):
        v, _ = fred_latest(sid, FRED_API_KEY)
        if v is not None:
            out["1Y"] = round(v, 2)
            break
    # For 2Y: interpolate between short-term and 10Y
    if "1Y" in out:
        v10, _ = fred_latest("INDIRLTLT01STM", FRED_API_KEY)
        if v10 is not None:
            out["2Y"] = round(out["1Y"] + (v10 - out["1Y"]) * (1.0 / 9.0), 2)
    return out
...
# Macro sources
_MACRO_SRC = {
    "India CPI": "https://fred.stlouisfed.org/series/INDCPIALLMINMEI",
    "India WPI": "",
    "India GDP": "https://data.worldbank.org/indicator/NY.GDP.MKTP.KD.ZG?locations=IN",
    "US CPI Index": "https://fred.stlouisfed.org/series/CPIAUCSL",
    "US Unemployment": "https://fred.stlouisfed.org/series/UNRATE",
    "India Forex": "https://fred.stlouisfed.org/series/TRESEGINM052N",
}
...
_logo_path = os.path.join(os.path.dirname(__file__) or ".", "advait_logo.png")
if os.path.exists(_logo_path):
    ...
    st.markdown(
        ...
        [data-testid="stAppViewContainer"]::before{{
            ...
            width:350px;height:80px;opacity:0.04;
            pointer-events:none;z-index:0;
            background:url("data:image/png;base64,{_logo_b64}") center/contain no-repeat;
        }}
        </style>''', unsafe_allow_html=True)
...
def mcard(name, value, change_html, src, anchor=""):
    ...
    return (f'{a_open}<div class="mcard"><div class="mname">{name}</div>'
            ...
            f'<div class="msrc">[{src}]</div></div>{a_close}')
def hl(name, tk, src, d, anchor=""):
    v = d.get(tk)
    return mcard(name, num(v['price']) if v else '-',
                 chg(v['d1']) if v else '', src, anchor)
cards = []
if eq.get("^NSEI"): cards.append(hl("Nifty 50", "^NSEI", "Y", eq, "sec-1"))
if eq.get("^GSPC"): cards.append(hl("S&P 500", "^GSPC", "Y", eq, "sec-1"))
if ty["10Y"][0] is not None:
    cards.append(mcard("US 10Y Yield", f"{ty['10Y'][0]:.2f}%",
                       chg(ty['10Y'][1]*100, bps=True, inverse=True), "T", "sec-2"))
if cm.get("GC=F"): cards.append(hl("Gold", "GC=F", "Y", cm, "sec-4"))
if fx.get("INR=X"): cards.append(hl("USD/INR", "INR=X", "Y", fx, "sec-3"))
if cards:
...
def _yield_cell(country, tenor):
    ...
    # 2. Browser scrape (Investing.com — works locally)
    bkey = f"{country}_{tenor.lower()}"
    v = _yld.get(bkey)
    if v is not None:
        return f"{v:.2f}%"

    # 3. FRED monthly (10Y only for most countries)
    if tenor == "10Y":
        return auto_val(f"{country}_10y")

    # 4. US Treasury.gov + FRED daily
    if country == "us":
        ...
        return f"{fv:.2f}%" if fv is not None else gap()
    ...
    if _frames:
        ...
        try:
            ...
            _line = (_alt.Chart(_long).mark_line(strokeWidth=1.6)
                     .encode(x=_x,
                             y=_alt.Y("yield:Q", title="10Y yield (%)",
...
rbi = bis_policy("IN")
st.markdown('<div class="sec-desc" style="margin-top:14px">The rate set by the central bank '
            '— it influences every loan, deposit and EMI in the country.</div>',
            unsafe_allow_html=True)
render_table(["Central bank", "Policy rate"], [
    [_link("RBI Repo Rate", "https://www.rbi.org.in/Scripts/BS_ViewBulletin.aspx"),
     rbi if rbi else gap("BIS feed unavailable")],
    [_link("US Fed Funds Rate", "https://fred.stlouisfed.org/series/FEDFUNDS"),
     f"{ff:.2f}%" if ff is not None else gap("add FRED key")],
])

# ---- 4. Currencies ----
report_section("3 · Currency markets")
_sec_desc("Exchange rates — how many rupees (or other currency) one dollar buys. A rising number means the rupee is weakening.")
rows = []
for name, tk in FX:
    d = fx.get(tk)
    cur = num(d["price"], 4) if d else gap()
    rows.append([_link(name, _SRC.get(tk)), cur] + pcells(d, ("d1", "m1", "m3", "m6", "y1", "y5"), "", 4))
...
# ---- 6. Alt assets ----
report_section("5 · Alternative assets")
_sec_desc("Crypto, REITs (real estate funds) and InvITs (infrastructure funds) — assets beyond traditional stocks and bonds.")
rows = []
...
_uvd = vl.get("^VIX")
_iv = (_ivd["price"], _ivd["d1"]) if _ivd else None
_uv = (_uvd["price"], _uvd["d1"]) if _uvd else None
render_table(["Indicator", "Current", "Change"], [
    [_link("India VIX", _SRC.get("^INDIAVIX")),
     num(_iv[0]) if _iv else gap(), chg(_iv[1]) if _iv else ""],
    [_link("US VIX (CBOE)", _SRC.get("^VIX")),
     num(_uv[0]) if _uv else gap(), chg(_uv[1]) if _uv else ""],
])


# ---- 8. Macro ----
report_section("7 · Macro indicators")
...
render_table(["Indicator", "Latest"], [
    [_link("India CPI", _MACRO_SRC["India CPI"]), auto_val("india_cpi")],
    ["India WPI inflation (YoY)", _wpi_cell],
    [_link("India GDP Growth", _MACRO_SRC["India GDP"]), _gdp_cell],
    [_link("US CPI Index", _MACRO_SRC["US CPI Index"]), f"{cpi:.2f}" if cpi is not None else gap("add FRED key")],
    [_link("US Unemployment %", _MACRO_SRC["US Unemployment"]), f"{un:.2f}" if un is not None else gap("add FRED key")],
    [_link("India Forex Reserves", _MACRO_SRC["India Forex"]), _resv_cell],
])
