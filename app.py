...
def build_pdf(report, meta, summary=None):
    ...
    _pdf_logo = os.path.join(os.path.dirname(__file__) or ".", "logo.png")
    _has_logo = os.path.exists(_pdf_logo)
    ...
    def _watermark(canvas, doc):
        if _has_logo:
            try:
                ...
                img = ImageReader(_pdf_logo)
                iw, ih = img.getSize()
                scale = (pw * 0.5) / iw
                dw, dh = iw * scale, ih * scale
                canvas.setFillAlpha(0.06)
                canvas.drawImage(_pdf_logo, (pw - dw) / 2, (ph - dh) / 2,
    ...
    if _has_logo:
        try:
            el.append(Image(_pdf_logo, width=120, height=26))
