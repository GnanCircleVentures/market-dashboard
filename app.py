...
render_table(["Indicator", "Current", "1D Change"], [
...
render_table(["Indicator", "Latest", "Period"], [
    [_link("India CPI", _MACRO_SRC["India CPI"]), auto_val("india_cpi"), "Monthly (YoY index)"],
    ["India WPI inflation (YoY)", _wpi_cell, "Monthly"],
    [_link("India GDP Growth", _MACRO_SRC["India GDP"]), _gdp_cell, "Quarterly / annual"],
    [_link("US CPI Index", _MACRO_SRC["US CPI Index"]), f"{cpi:.2f}" if cpi is not None else gap("add FRED key"), "Monthly"],
    [_link("US Unemployment %", _MACRO_SRC["US Unemployment"]), f"{un:.2f}" if un is not None else gap("add FRED key"), "Monthly"],
    [_link("India Forex Reserves", _MACRO_SRC["India Forex"]), _resv_cell, "Monthly"],
])
